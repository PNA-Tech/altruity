<script lang="ts">
  ...
  async function load() {
    ...
    unsubscribeFollowerCount = await pb
      .collection("users")
      .subscribe(user.id, async (rec) => {
    ...
    followerUsers = (
      await pb
        .collection("users")
        .getFullList(50, { filter: `following.id ?= "${user.id}"` })
    ).map((l) => l.id);
    unsubscribeFollowingCount = await pb
      .collection("users")
      .subscribe("*", async (rec) => {
        if (
          !followerUsers.includes(rec.record.id) &&
          rec.record.following.includes(user.id)
        ) {
  ...
  async function follow() {
    ...
    if (followerUsers.includes($loggedUser!.id)) {
      await pb.collection("users").update($loggedUser!.id, {
        following: $loggedUser!.following.filter((l: string) => l != user.id),
      });
    } else {
      await pb.collection("users").update($loggedUser!.id, {
  ...
  function unload() {
    ...
    if (unsubscribeFollowerCount) {
      unsubscribeFollowerCount();
    }
    if (unsubscribeFollowingCount) {
...
<div class="row mb-3">
  ...
  {#if $loggedUser && $loggedUser.id != user.id}
    <button
      ...
      on:click={follow}
      >{followerUsers.includes(asRecord($loggedUser).id)
        ? "Unfollow"
        : "Follow"}</button
    >
  {:else if $loggedUser && $loggedUser.id == user.id}
    <a class="col-3 btn btn-outline-primary me-2" href="/settings">
      <div class="h-100 lgbtn">
        <span class="d-flex"><i class="bi bi-pencil-square me-2" />Edit</span>
      </div>
    </a>
  {/if}
</div>
